        ...
        for u in range(len(self.subjects_split[self.set])):
            ...
            if self.chunk_level_datapoints:
                ...
                for i in range(nr_post_groups):
                    ...
                    self.indexes_per_user[u].append(range(i * self.posts_per_group + self.post_offset, min((i + 1) * self.posts_per_group + self.post_offset, len(user_posts))))
                    self.indexes_with_user.append((u, range(i * self.posts_per_group, min((i + 1) * self.posts_per_group + self.post_offset, len(user_posts)))))
            else:
                ...
                for i in range(nr_post_groups):
                    ...
                    self.indexes_with_user.append((u, range(i, min(i + self.posts_per_group + self.post_offset, len(user_posts)))))
