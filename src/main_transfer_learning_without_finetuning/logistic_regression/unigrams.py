...
if __name__ == '__main__':
    ...
    hyperparams_features = {"vocabulary_path": f"../../../resources/generated/vocab_daic/unigrams_participant_3123.txt"}


    hyperparams["positive_class_weight"] = 3
    hyperparams["chunk_size"] = 300
    hyperparams["batch_size"] = 16
    hyperparams["learning_rate"] = 0.1

    data_generator_train_source, data_generator_valid_source, data_generator_test_source = initialize_datasets_unigrams(user_level_data_source,
                                                                                                                      subjects_split_source,
                                                                                                                      hyperparams,
                                                                                                                      hyperparams_features)

    data_generator_train_target, data_generator_valid_target, data_generator_test_target = initialize_datasets_unigrams(user_level_data_target,
                                                                                                                      subjects_split_target,
                                                                                                                      hyperparams,
                                                                                                                      hyperparams_features)
    hyperparams_features["embedding_dim"] = data_generator_train_source.get_input_dimension()
